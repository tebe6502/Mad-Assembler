...
//---------------------------------------------------------------------
// SOFTWARE SPRITES ENGINE II EXAMPLE (CHARS MODE) v1.0 (19.10.2008)
//---------------------------------------------------------------------
	...
	icl 'CharEngine_3xBuf\Engine_3xBuf.hea'
...
dlist0	@DLIST PlayfieldB0+4*PlayfieldWidth+4, dlist0

dlist1	@DLIST PlayfieldB1+4*PlayfieldWidth+4, dlist1
...
tColor1Addr	.word tColor1		; możliwość przemieszczania adresu w przypadku scrolla pionowego
tColor2Addr	.word tColor2		; możliwość przemieszczania adresu w przypadku scrolla pionowego
tColor3Addr	.word tColor3		; możliwość przemieszczania adresu w przypadku scrolla pionowego
...
chr	lda #$ff	;$d20a
	...
	ldy #0
	lda #1
f0	sta PlayfieldBuf,y
f1	sta PlayfieldB0,y
f2	sta PlayfieldB1,y
	eor #$80
	iny
	bne f0
	inc f0+2
...
vbl	phr
	...
	ift PlayfieldWidth-8=32
	lda #scr32
	eli PlayfieldWidth-8=40
	lda #scr40
	els
	ert 1=1
	eif
...
b7	@@CutMIC 27 21 3 21
	...
	.align
; !!! koniecznie od początku strony pamięci !!!

EngineProgram
	
	.link 'CharEngine_3xBuf\CharEngine_3xBuf.obx'	
