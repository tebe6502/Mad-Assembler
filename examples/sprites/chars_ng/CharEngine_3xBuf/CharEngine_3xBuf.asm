...
; ZAŁOŻENIA:
; - możliwość zdefiniowana liczby zestawów znakowych zmienianych co wiersz (tablica CHARSETS) na przerwaniu DLI
...
; - stała liczba duchów = 7
; - stały rozmiar duchów = 12x21 pixle
; - dwa bufory dla pamięci obrazu, w każdym z nich możliwe jest użycie znaków 0..70
; - tylko 1 bitmapa kształtu dla 1 klatki ducha, przesuwanie bitów realizowane poprzez tablicę
; - bitmapa maski obliczana na podstawie aktualnej bitmapy kształtu poprzez tablicę (nie ma potrzeby jej przesuwać)
; - stałe wartości kodów znaków dla reprezentacji duchów

; ZALETY:
; - brak potrzeby rozpisania klatek kształtu i maski ducha, oszczędność pamięci

; WADY:
; - 7 duchów wyrabia się w 2 ramkach na wąskim ekranie (silnik z gry BombJack obsłuży w tym czasie 11 duchów)
...
; znak $7f nie może zostać użyty bo zostanie zniszczony obszar $fff8..$ffff z wektorami przerwań
...
buf	lda #0				; przełączanie buforów
	tax
	eor #1
	sta buf+1	
	...
	cpx #0
	sne
	jmp PlayfieldB1_Restore		; usunięcie duchów w buforze #1, wyświetlany jest bufor #0
	jmp PlayfieldB0_Restore		; usuniecie duchów w buforze #0, wyświetlany jest bufor #1
...
RESET	lda #0				; wszystkie duchy wyłączone
	sta Sprite0.x
	sta Sprite1.x
	sta Sprite2.x
	sta Sprite3.x
	...
	lda <PlayfieldB0		; poprzedni adres ducha dla BUF#0 ustawiamy na lewy górny narożnik
	sta zp+@zp.old0B0
	sta zp+@zp.old1B0
	sta zp+@zp.old2B0
	sta zp+@zp.old3B0
	...
	lda <PlayfieldB1		; poprzedni adres ducha dla BUF#1 ustawiamy na lewy górny narożnik
	sta zp+@zp.old0B1
	sta zp+@zp.old1B1
	sta zp+@zp.old2B1
	sta zp+@zp.old3B1
...
.macro	@sprite_init
	...
	sty Sprite:2.xOk		; aktualnie obowiązująca pozycja pozioma ducha
	sta Sprite:2.yOk		; aktualnie obowiązująca pozycja pionowa ducha
